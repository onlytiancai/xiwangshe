...
class message(dict):
    ...
    def send_response(self, status, body=''):
        ...
        if len(to_send) > MAX_MSG_SIZE: raise ValueError('to send data overflow')
...
class parser(object):
    ...
    def _make_request(result, input):
        return message(message_type=message.REQUEST, method=result.group(1), seq=result.group(2), body=input.read(), raw=result.group())

    @staticmethod
    def _make_response(result, input):
        return message(message_type=message.RESPONSE, status=int(result.group(1)), seq=result.group(2), body=input.read(), raw=result.group())

    @staticmethod
    def _make_notify(method, input):
        return message(message_type=message.NOTIFY, method=method, body=input.read(), raw=method)
...
class client(object):
    ...
    def check_timeout_process():
        ...
        while True:
            try:
                now = datetime.now()
                to_removes = []
                logging.debug('check timeout:%s %s', now, len(client.sent_requests))
    ...
    def send_request(url, method, body='', timeout=30):
        ...
        msg = message(message_type=message.REQUEST, method=method, seq=seq, body=body)
